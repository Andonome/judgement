...
\subsection{Describe Back-to-Front}
...
The players won't notice the armour with guild insignia, or the potions on the table.
...
If I ever see a player point towards something, I know they've joined me `in the game'.
...
Games with revolving players can easily handle a dozen players.

Scheduling problems don't interfere with other hobbies nearly as much as tabletop RPGs, but it has nothing to do with the people.
...
And if you -- the \gls{gm} -- find one week you can't make it, just ask if anyone else wants to run a one-shot.
Nobody's irreplaceable, not even \glspl{gm}.
...
\footnote{See \autopageref{seasons} for a breakdown of \gls{fenestra}'s seasons.}
...
If your troupe insist on acting in `order of Initiative', you can help them along by hollering the \gls{ap} count, from highest to lowest.
...
Don't fear killing the whole troupe -- if some creature kills a party member, it will typically eat them rather than run after the rest; and if sentient enemies kill \pgls{pc}, just have the rest capture the survivors and go from there.
...
If a \gls{pc} is totally out of commission, with 1 \gls{hp} left, 4 \glspl{fatigue} from being bled dry, and an inexplicable curse, consider letting them play \pgls{npc} and letting them keep all \glspl{xp} gained during this time.
