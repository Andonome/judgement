...
Find \pgls{gm}'s note-sheet \vpageref{gmSheet}.
...
\subsubsection{Deciding Bonuses}
\index{Bonuses, Environmental}
\index{Environmental Bonuses}

A list of every possible item, and the Bonus it might give to every conceivable roll, would be too heavy to transport to any table.
But if that list existed, it would use the default of `\gls{weight}~1 = +1~Bonus'.
...
I don't have a rule for environmental Bonuses, except that you should apply them liberally.
...
\begin{itemize}
  \item
  Start out with \glspl{guard}, and if \pgls{pc} kills \pgls{jotter}, switch the entire \gls{campaign} to banditry, with wanted posters, questions of hygiene while on the run, and the looming threat of frost.
  \item
