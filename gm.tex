...
\begin{itemize}
  ...
  \item
  Coins or cards to track \glspl{hp}, \glspl{fp}, and any other points.
  \item
  A blade.
  \item
  The \gls{gm} sheet, to track whatever comes up.
  Take your blade, and slice it from the back of this book (or just get a notepad).
  \item
  Some encounters.
  Either use the \gls{gm}-shield (QR code at the back) or the Almanac.
  Both change every week, updating with the weather and creatures which are active in \gls{fenestra}.
  \item
...
And coins mean the \gls{gm} can physically hand people the \glsentrylongpl{ep} gained from a long night's \gls{vigil}.

As \pgls{gm}, it's always good to have at least three different types of coins -- Damage, \glsentrylongpl{ep}, and \gls{weight}.
The exact coins don't matter, as long as they remain consistent through \pgls{campaign}.
You might use `gold' for Damage, `silver' for \glsentrylongpl{ep}, and copper for the rest.
The character sheets have circles which fit \euro 0.05, so most coins should fit comfortably.

Multiple coin-types can also help \pgls{gm} track many \glspl{npc}.
Let's say you're orchestrating a battle with a hobgoblin leader, some hobgoblin troupes and a goblin spellcaster.
...
is \vpageref{gmSheet}.
In the centre, you have a horrifying problem -- you can either write down the currently active \glspl{pc}, and hope to note them elsewhere after each session, so you can comfortably update this space; or you can try to write in delicate mouse-letters, in order to fit every \gls{pc} who ever joins in the session.
...
If you want an easy excuse, tell the players that the local \gls{jotter} has fallen behind on their paperwork, and wants the troupe to give them a complete report of their last mission.
...
Putting the background first also makes it easier to remember those little details, like how a tavern smells, or the flowers on a hill.
...
\textit{\Large\raggedright `Branches crack all around the forest, always somewhere far away, like distant snoring.'}

\textit{\large\raggedright `Every second step, your \gls{torch} helps you navigate, but also alerts everything around you with its light.'}
...
\begin{boxtext}
  \Pgls{soldier} stops wrestling his donkey, and runs towards the \gls{jotter}, arguing that he and his crew should go first.
...
\Pgls{npc} attacks the \glspl{pc}, but you have no idea \textit{which} \gls{pc} they would attack first, have them attack whoever has the most \glsentrylongpl{fp}, then keep attacking that \gls{pc} until they have a reason to change their behaviour.

\Pgls{pc} asks if the cavern floor has any large rocks.
That seems plausible, but not likely, so roll a D6, and on the roll of a `6', the \gls{pc} finds what they're looking for.

\Pgls{seeker} finds the \glspl{pc} in an abandoned city, but would they trust these \glspl{guard} enough to approach them?
Roll $2D6$, and have them flee if they roll above their own \glspl{hp}, otherwise they approach (just like the morale roll, \vpageref{morale}).
...
\begin{speechtext}
  So while the other characters attack for a few rounds, you want your character to go through their backpack to get their tinder box and \pgls{torch}, then use the tinder box to light the \gls{torch}.
...
The gaming table won't feel `full' when yet another person joins the \gls{campaign}.
...
\subsubsection{Running \Glsfmtplural{action} in Parallel}
can easily double the pace of combat whenever the troupe face a lot of \glspl{npc}.
As long as the \glspl{npc} have more \glspl{ap} than the \glspl{pc}, ask every player to make a combat roll \emph{at the same time}.

\begin{speechtext}
  With two gnolls jumping out behind, everyone finds themselves facing one.
  Everyone roll at \tn[11], and remember to roll Damage at the same time.
\end{speechtext}

Whenever a player says they deal Damage, remove a gnoll's \glspl{hp}.
When they miss, then that Damage die applies to the player's character.

Once the table gets used to it, you can often resolve \pgls{round} in two rolls and a couple of pencil-strokes.
...
in the \gls{court} if a player feels unsure of the \gls{combat} rules.
The next time they pay to see justice being done, hand two players \pgls{statblock} each and let them battle to the death with a couple of rolls.
Some people find it easier to play with the system when their character isn't about to die.
...
\Gls{bind}'s armour basically reduces everything to `basic armour covers 3 points, full armour covers 5 points', so it won't represent the finer points of brigandine vs chainmail.
...
is a massive, screaming deal in \gls{bind}.
...
The new \glspl{pc} begins with a number of \glspl{storypoint} equal to the previous character's $\frac{\mbox{\glspl{xp}}}{10}$
(minimum of 5).%
...
don't have an asterisk next to them.
Rules and procedures are noted side-by-side, with the assumption that people will follow both when starting, and hopefully break from the procedures when they stop working.

The \gls{gm}'s procedures include Morale Checks, \glspl{thread}, missions, weather charts, and \glspl{fiend}.
These procedures should provide fuel and a safety net, but need adjustmented or discarded routinely.
Rolling Morale checks for \glspl{crawler} won't cause problems (animals can be unpredictable, and it makes sense that they don't always make sense), but sticking to Morale checks for bandits might suggest strange actions for a rational person to take.
\Glspl{segment} need even more adjustments --- every \gls{segment} should be looked at with suspicion just before running it.

Players should find themselves so wrapped up in meaningless procedures that it forces them to break out and challenge how the world works.
Starting characters in the \gls{guard} don't have the weaponry to take on monsters, so they may have to steal, beg, or flee.
Their taxes stop them buying better weapons, the \glspl{jotter} don't care if they live or die, and two of the great \glspl{temple} want to claim copyright on vinegar.

If a missions conflicts with \pgls{thread}, you don't have to cut one in favour of the other in order to make sure that the \glspl{pc} can do both.
Just make it clear that they will have to select one goal or the other, then roll a random encounter.

\subsubsection{Writing \Glsfmtplural{statblock}}
doesn't have any reason or procedure beyond what makes sense for the creature.
If \pgls{npc} has nothing noted, just write their \glspl{trait} on-the-fly to keep them consistent.
\index{Statblocks@\Glsfmtplural{statblock}}

\subsubsection{Leave the \Glsfmttext{guard}}
and focus the \gls{campaign} on the most interesting parts.
\Gls{bind} only focusses on the \gls{guard} as a convenient plot hook, but your nights don't \emph{need} to revolve around them.
...
When you make a settlement by a lake, you might make a group of `boat humans', who consider Seafaring to be a standard \gls{skill} for everyone; or make `stone elves', who live across a particular mountain range, partially underground.

We don't need any new rules for these distinctions --- shared languages, and \glspl{skill} give enough bare-bones mechanics to hang a story on top of.

\subsection{I Lied}
I lied about how lethal \gls{bind} is.
...
\subsection{The Metaphysics of Being a Tree}
...
Anyone transformed to stone would retain their organs with the same \textit{telos}, meaning that a liver would continue to process toxins, and the eyes continue to see, because that's what eyes are for, regardless of their `substantive cause' (meaning, the substance they are made from).
