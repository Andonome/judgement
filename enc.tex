...
\index{Encounters!Random}

Find the next encounter, when it occurs, and the weather, with a single roll of $4D6$.
The interpretation depends on the \gls{cycle}.
...
The most common things one encounters are \emph{traders}.  Each morning and
afternoon, roll a die; if the die rolls equal or less than the
\textit{Civilization Rating} then the troupe encounter traders.

Out on \pgls{lonelyRoad}, the troupe encounter traders on the roll of `1'.  The
closer they come to a town, the more chance of finding traders.
...
\begin{boxtable}
  \bf Rating & \bf Type of Place \\\hline
  \dicef{\arabic{dlist}} &
  Means the troupe is in a forest, far from any reasonable location.
  \stepcounter{dlist}
  \stepcounter{encnum}
  \\
  \arabic{dlist} &
  \encCivilization, between two \glspl{village}, perhaps by \pgls{bothy}.
  \stepcounter{encnum}
  \stepcounter{dlist}
  \\
  \arabic{dlist} &
  \encCivilization\ traders arrive on the roll of `\arabic{dlist}' because they
  often need to visit to ask for an escort, but dislike wandering out where to
  many \glspl{monster} roam.
  \stepcounter{encnum}
  \stepcounter{dlist}
  \\
  \arabic{dlist} &
  \encCivilization\ have plenty of traders dropping by every few days.
  \stepcounter{encnum}
  \stepcounter{dlist}
  \\

  \arabic{dlist} &
  \encCivilization\ often have one side looking towards open fields, with peaceful hamlets in the distance.
  \stepcounter{encnum}
  \stepcounter{dlist}
  \\

  \arabic{dlist} &
  \encCivilization\ \glspl{monster} become rare, as most never make it this
  far.
  \stepcounter{encnum}
  \stepcounter{dlist}
  \\
...
\label{traders}
depend on the other dice.  Roll 3 more to determine what the traders have which
might interest the \glspl{pc}.  Traders always shout this information ahead of
them, to entice people to buy their goods.
...
Traders also purchase live \glspl{monster} for a number of \glspl{sp} equal to
their \gls{cr}, or half that number if the body is not well prepared (for
example, if the \glspl{pc} failed their \gls{harvesting} roll).  Legally, the
troupe should give half of this money to the \gls{templeOfBeasts}, but that's
\emph{their business} --- most traders don't care about another guild's laws
once they're outside a town.

\subsubsection{The Time Until the Encounter}
depends on the season, just like creature-encounters, \vpageref{creatureEncounters}.

\needspace{8em}
\subsection{Creatures}
\label{creatureEncounters}
...
encounters use the first $1D6$ to find the number of days until the encounter, the second $1D6$ for the result, and the third to determine the number.%
...
Rolling \dicef{4}~\dicef{2}~\dicef{1}~\dicef{3} would means `after 4 days, the \glspl{pc} encounter a goat during the morning'.
...
Add the middle pair to determine the encounter.
...
\boxPair{
  ...
  \begin{exampletext}
    ...
    One soggy~\gls{cOne}, the \glspl{pc} emerge from \pgls{broch}, and head towards \pgls{village}.
...
\index{Morale}
start before combat.
The \gls{gm} rolls $2D6$ and leaves the roll on the table.
While the \gls{monster}'s has more \glspl{hp} than the roll, it attacks;
if its \glspl{hp} fall below the roll, it flees from combat or watches quietly.
...
When a tie occurs in the middle of combat, this can still prompt creatures to back off for a moment and reassess the situation (with another morale check).
These tense stand-off moments can take a while, so the \glspl{pc} can take $1D6$~\glspl{round} to prepare (perhaps reloading crossbows as \pgls{monster} rears)
but any movement away or towards the enemy prompts an immediate resolution.
...
\noindent
The \glspl{pc} will find more trouble than \glspl{monster} as they travel.
Wherever a journey ends, a situation begins.
...
\begin{dlist}
  \item
  A retired \gls{thane}, with a hundred war-stories.
  Of course when he tells them, nobody can get a drink, so don't ask!
  \item
...
\begin{dlist}
  \item
  An elf who doesn't speak \gls{tradeTongue}.
  \item
  \ifcase\value{r0t3}%
    A blind and cheerful
  \or%
    An obsessive and narrow-minded
  \or%
    A deaf but talkative
  \else%
    A one-legged, singing,
  \fi%
  \gls{mixer} from the \gls{templeOfSickness}, on a mission to recruit from the \glspl{village}.
  \item
...
(see \autopageref{pitOfJustice} for the building's structure).
