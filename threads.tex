...
\declareRegions{Forest,Town,Roads}
...
To illustrate, I'm going to take some classic stories, and rearrange them as \glspl{thread}, just to show how they look.

\thread[Forest,Roads]{Three Little Pigs}
\label{piggies}

\noindent
Each \gls{thread} begins with a summary of its \gls{segment}.
...
{A little pig invites the troupe to dinner, just before the big, bad, wolf summons a storm to collapse the building}% SUMMARY
...
This \gls{segment} may take place the next day, or the next week.
If the \glspl{pc} remain the forest, they will find other \glspl{segment} from different \glspl{thread}.
The next \gls{segment} in `\nameref{piggies}' simply waits for them, patiently.

As before, this house might be anywhere on the roads, so you can start the \gls{segment} and place the house on the map no matter where on the roads the \glspl{pc} wander.

\begin{exampletext}
  The wolf has tracked down another little pig, but waits until night to summon a storm, and collapse the roof.
\end{exampletext}
...
then you can pull up the next \gls{thread}'s \gls{segment}, and start it immediately, interrupting the fight with the wolf.
This mixing of various plots helps weave the stories into a tapestry.

\paragraph{If the \glspl{pc} kill the big, bad, wolf}
then the \gls{thread} stops here.
\Glspl{thread} sometimes do that, and that's okay -- sometimes \pgls{thread} finishes early.

\paragraph{If the \glspl{pc} simply leave}
then the pig dies, and the \glspl{pc} can find his collapsed house the next time they pass by.
...
Each \gls{thread} with a creature should always contain a full statblock on the same page or the next page, so you can see all the information required for the scene at a glance.
...
\thread[Forest,Roads,Town]{Cinderella}
...
However the scene ends, the important part is just to establish the existence of Cinderella.
...
\thread[Forest,Roads,Town]{Snow White}
...
If they press her, she sells them one day's worth of food, and after \pgls{interval} they begin to suffer \glspl{ep}.
...
\section{Plotting \Glsfmtplural{thread}}

\begin{multicols}{2}

\noindent
Juggling all these \glspl{thread} and \glspl{segment} threatens to become confusing, so each collection comes with a summary which plots out the \glspl{segment} \emph{per location}.

Have a look at the summaries below.
Notice that the first \gls{segment} comes with the `\sgr' symbol.
Once it's done, you can mark it off with an `X', then mark the next \gls{segment} in the \gls{thread} as ready with a `\sgr'.

Imagine what would happen if the troupe stick around on the Roads for a while; they could fly through the \glspl{segment} from the first \gls{thread}, before the second arrives as a distraction.
...
\begin{enumerate}
  \item
  The \gls{segment} must make sense anywhere within its \gls{region}.
  \item
...
\subsection{\Glsfmtplural{region} Make the \Glsfmttext{campaign}}

\Pgls{campaign} can focus on particular places by selecting the right \glspl{region}.
\Glspl{thread} which use `Forest, Roads, Town', as their \glspl{region}, will focus on overland travel, because the troupe will have to travel the wide lands between those places.
...
\noindent
As the \glspl{pc} move around, they will slowly `trigger' new \glspl{thread} until you have four or five \glspl{thread} all activated at once.
This may sound like it will confuse the players, but it won't, because they won't be aware of any \glspl{thread} -- they simply see individual \glspl{segment}, and most of the early \glspl{segment} only serve as foreshadowing.

All these tangled threads usually result in players seeing just one or two `main plots' at a time.
In some sense, they are always correct -- if the players take \pgls{thread} as the main one, then it has become their primary \gls{thread}.
But once that `main' \gls{thread} resolves, others already have their ground-work laid out, in the form of rumours or short encounters with \glspl{npc} who will later become important.

However, you can still get too much of a good thing.
In order to wrap up \pgls{campaign} cleanly, you need some `plot-buffer', where no more \glspl{thread} begin.
You need \pgls{thread} with nothing but random, unconnected events in the \gls{region}.

Check the summary of \glspl{segment} in each \gls{region}, \vpageref{sideQuestSummaries}, and imagine players deciding to go from one area to the next.
...
However, on the roads you will have plenty of material for the players, even if none of the big-and-relevant plots have \pgls{segment} ready.

\thread[Roads]{Random Events on the Road}
...
\subsection{Encounters as \Glsfmtplural{segment}}

\Glspl{gm} who like to prepare random encounters before a game can use them like any other \gls{thread}.
Just roll a few encounters%
\footnote{Find the chart \vpageref{randomEncounters}.}
...
\footnote{Details \vpageref{civilizationRating}.}
...
\footnote{Find random missions \vpageref{NGmissions}, or just make some up.}
Once the \glspl{pc} arrive in \pgls{broch}, the local \gls{jotter} starts handing out missions.
