...
\setcounter{season}{7}
%\setcounter{diceNo}{\day}
\setcounter{diceNo}{22}

\newcounter{ainumarPosition}
\setcounter{ainumarPosition}{30}
\newcounter{fenestraPosition}
\setcounter{fenestraPosition}{12}
\multiply\value{ainumarPosition} by \value{season}
\multiply\value{fenestraPosition} by \value{diceNo}
\addtocounter{diceNo}{0}
\addtocounter{ainumarPosition}{0}
...
\boxPair{
  \begin{tikzpicture}
    ...
    \coordinate [label=\outline{\Large \glsentrytext{ainumar}}] (AN) at (A)+(3,1);
    \coordinate [label=\outline{\glsentrytext{fenestra}}] (FN) at (F)+(2.3,0.3);
  \end{tikzpicture}
  ...
  \begin{nametable}[l|Y|c]{Seasons}
    ...
    \Repeat{12}{
      \addtocounter{gold}{7}%
      \ifnum\value{gold}>28%
        \addtocounter{gold}{-28}%
        \addtocounter{track}{1}%
      \fi%
      \trackMonth~\arabic{gold} &
      \setCycle{\value{track}}{\value{gold}}%
      \setTemperature%
      \showCycle, 
      days \ifodd\value{gold} 1 to 30\else 30 to 60\fi %
      & \showTemperature
      \\
    }
  \end{nametable}
...
\subsubsection{The \Glsfmttext{ainumar}}
...
\subsubsection{\Glsfmttext{fenestra}}
\index{Seasons}
...
After three years, \gls{ainumar} completes \pgls{cycle}, and the twelve seasons begin again.
...
For a full list of the events, check \vpageref{weather}.
