...
\Gls{harvesting} them like this requires processing the piece into something useful (e.g. \pgls{boon}, or \pgls{talisman}) before it rots, and the rate of rot depend on the season.%
\footnote{See \autopageref{manaIndex} rot rates.}
...
\begin{itemize}
  \item
  Flat, open, plains, with few bushes and low-cut grass.
  Aurochs cut paths for themselves through the forest.
  \item
  Bones warn that some predator waits in the area.
  \item
  Deep hoof-prints, leaving a muddy trail.
\end{itemize}

\index[mana]{Earth!Hoof of auroch|glsentrytext{A}}
\paragraph{\Glspl{ingredient}:}
a dozen auroch hooves, crushed into a fine powder, provide \pgls{ingredient} of the Earth \gls{sphere}.
...
\makeRule{stampede}{Avoiding a Stampede}{Speed}{Athletics}{9}
Failure inflicts one Damage for every ten members of the herd, so a herd of 80 would inflict $\dmg{4}$~Damage.
A tie might indicate the character loses an item, or becomes trapped.
...
They grow to the length of four horses long, with six powerful legs propelling their huge bodies forward like a centipede.
...
\begin{boxtext}
  ...
  The troupe draw up their tents extra quickly, and move away from the wind as snapping branches can be heard in the far distance.
...
\paragraph{Natural Abilities:} \pgls{basilisk}'s odour forces anyone smelling it to make a Strength + Survival check, each round, at a \gls{tn} of 10 minus the creature's Speed, or gain 3 \glspl{ep} as the character wretches or simply stares into space with disgust.
...
\Glspl{basilisk} hibernate over \gls{cTwo}, often in a pile in caverns, where \glspl{olm} pick at any food-scraps in their teeth.

\paragraph{Signs:}

\begin{itemize}
  \item
  A long turd with a white, crystalline tip.
  \item
  Footprints like a flock of giant chickens.
  \item
  Scratchmarks on trees, where \pgls{basilisk} sharpened its claws.
  \item
  A stream's water tastes foul (\pgls{basilisk} waded thorugh it).
\end{itemize}

\index[mana]{Air!Gullet of \glsentrytext{basilisk}|glsentrytext{A}}
\index[mana]{Earth!Hide of \glsentrytext{basilisk}|glsentrytext{A}}
\paragraph{\Glspl{ingredient}:}
...
\index[mana]{Fate!Egg-tooth of \glsentrytext{basilisk}|glsentrytext{A}}
Newborn \glspl{basilisk}' `egg-tooth' provides a Fate \gls{ingredient} (with \pgls{weight} of~0).
...
\begin{itemize}
  \item
  Local farmers mention a bear stealing porridge.
  \item
  Fishermen by the inner hamlets complain about the lack of fish.
\end{itemize}

\index[mana]{Fire!Heart of bear|glsentrytext{A}}
\paragraph{\Glspl{ingredient}:}
...
\index[mana]{Fate!Sack of bluepin butterflies|glsentrytext{Sw}}

Laying a trap with sweet-smelling smoke can summon enough bluepins to make a Fate \gls{ingredient}.
The trap requires an \roll{Intelligence}{Survival} roll (\tn[12]).
On a tie, the bluepin population dwindles, and this \gls{region} will not see them again for $1D6$ years.
...
\begin{itemize}
  \item
  Several boar tracks lead the way to a patch of \glspl{marchingMushroom}.
  \item
  Two boar tracks, trampled by \gls{crawler} tracks, show a recent chase.
  The young one didn't make it.
\end{itemize}
...
Noticing the web before stepping into it requires a \roll{Wits}{Vigilance} action, \tn[7] during the daylight and higher at night or in the twilight of a forest.
Breaking free of the web requires a \roll{Strength}{Athletics} action at \tn[8] plus the \gls{crawler}'s Strength.
...
The female relies on \glspl{ambush}, while the male attempts to herd food towards her.
During this time, \glspl{crawler} change from killing their prey, to `stab-and-grabs', where they stab their prey once, grab it,%
\exRef{core}{the Core rules}{grab}
then dump it on a web to attract a mate, or to prepare for eggs to hatch.
...
\index[mana]{Fate!Spinnaret of \glsfmttext{crawler}|glsentrytext{A}}

\paragraph{Signs:}

\begin{itemize}
  \item
  A patch of floating leaves indicates an abandoned, degrading web.
  \item
  Perfectly clean, white, bones, and tattered leather coat in a ragged pile.
  \item
  \Pgls{crawler} carcass hangs upside-down from a tree; this one never woke from hibernation last \gls{cTwo}.
\end{itemize}
...
\newBeast{\Glsfmtplural{griffin}}% Name
  {griffin}% label
...
\index[mana]{Air!Wing of \glsentrytext{griffin}|glsentrytext{A}}

\paragraph{Signs:}

\begin{itemize}
  \item
  Three fly overhead, from a mountain cave, to hunt a distant auroch herd.
  \item
  Three nests hold nothing but snow.
\end{itemize}
...
Their greatest strength is that, while lying mostly underground, they are almost scentless, so creatures which co\"ordinate by their nose have a hard time spotting the lethal trap in waiting.
...
\begin{itemize}
  \item
  Distressed tree-roots jut upwards, showing tunnels beneath.
  \item
  Griffins `fish' for a digger by dropping a dead rabbit by a bush in the hopes of tempting it from its hole.
\end{itemize}

\index[mana]{Earth!Liver of \glsentrytext{digger} young|glsentrytext{A}}
\paragraph{\Glspl{ingredient}:}
...
\index[mana]{Fire!Queen of stirge swarm|glsentrytext{Sw}}
\paragraph{\Glspl{ingredient}:}
...
\begin{itemize}
  \item
  Outside \pgls{bothy}, horses cry out during the night.
  They smell nearby wolves.
  \item
  Howls at Dusk remind local farmers to double-check the locks on the sheep-pen.
\end{itemize}
...
\newBeast{\Glsfmtplural{woodspy}}% Name
  {woodspy}% label
...
\index[mana]{Water!Beak of \glsentrytext{woodspy}|glsentrytext{E}}
\paragraph{\Glspl{ingredient}:}
...
\begin{itemize}
  \item
  Slashes through the mud, like a drunken wagon with disconnected wheels.
  \item
  An upturned, bloody, skull, with the brains scouped out.
  \item
  Runny poop with the smaller bones --- spine segments, little ribs, and fingers.
\end{itemize}
...
\section[\Glsfmttext{deep} Creatures]{\Glsfmttext{deep} Creatures \A}
\index{Caving!Creatures}
...
Any passage leading downwards eventually sucks down any organic matter, and usually produces a slimy slope -- a natural trap, which pulls people into the \gls{deep} below.
...
This freezing, frostless, \gls{deep}, creates a game with strict rules for all its inhabitants.
...
Far below the frigid \gls{deep}, the air becomes warm again.
...
Some ooze also have the Projectiles \gls{skill}, indicating they can shoot acid up to 5~\glspl{step} away.
...
Once a target is grappled, the ooze inflicts damage equal to its Strength immediately, and again at the start of each \gls{round}.
...
\index[mana]{Fate!Urn of black or brown oozes|glsentrytext{Sw}}
\paragraph{\Glspl{ingredient}:}
...
\begin{itemize}
  \item
  An immaculately clean cavern suggests an oozes has swept up all debris.
  \item
  `Black perls' of undigestible waste such as dirt, rocks, or the metal parts of a dagger lie in a pile of three or four, each one a perfect sphere.
\end{itemize}
...
If a troupe communicate that they have become lost, the \glspl{sporefolk} will take this as a sign of malicious intentions (because anyone who becomes lost does not know the proper way to act), while giving gifts to them will make them inclined to believe statements the troupe make (because people who know good manners should also know what they are talking about in general).

\paragraph{Ecology:}
\Glspl{sporefolk} colonies are smart enough to remain untroubled by most underground creatures.
Their primary enemy is gnomes, who struggle to resist the temptation to study and pull apart these living \glspl{artefact}.

Once dead, \glspl{sporefolk} release any \glsentrylongpl{mp} in their bodies.

\index[mana]{Fate!Body of \glsfmttext{sporefolk}|glsentrytext{E}}
\paragraph{\Glspl{ingredient}:}
\glspl{sporefolk} bodies can be used as Fate~\glspl{ingredient}.
...
\begin{itemize}
  \item
  `Root-marks', where \pgls{sporefolk} absorbed some nutrients from the soil look like shoddy mandalas.
  \item
  \Glspl{sporefolk} write with divets, holes, and other subtle marks, easiy dismissed as natural formations.
\end{itemize}
...
\begin{dlist}
  ...
  The fungus may or may not be edible (\glspl{sporefolk} don't know much about what flesh-people like to eat).
  \item
  They try to trade.
  ...
  The troupe probably don't have time for this, but the \glspl{sporefolk} will persevere as long as the troupe remain, and will not let them sleep.
\end{dlist}
...
\newBeast{\Glsfmtplural{hulk}}% Name
  {umber_hulk}% label
\glsentrydesc{hulk}.
...
\paragraph{Natural Abilities:} When stressed, angry, or horny, a female \gls{hulk}'s odour forces anyone smelling it to make a Strength check, each \gls{round}, at \tn[10] minus the creature's Speed, or gain 3 \glspl{ep} as the character wretches.
This stench quickly fills tunnels, and can serve as an early warning sign, but not always.
The stench only occurs when the creature starts galloping, but it strong enough to emanate out of cave mouths miles above.

The odour attracts any local males, who have no such odour, but do have wings hidden under their shells.
They use these wings almost exclusively over \gls{cFive}, when males search for a mate by flying around \gls{fenestra}'s mountains.
...
Due to heat encouraging the eggs to hatch, anyone wandering nearby with \pgls{torch} often encourages the entire brood to exit their shells immediately, and cry for their parents.
...
\index[mana]{Fire!Egg of \glsentrytext{hulk}|glsentrytext{A}}
\paragraph{\Glspl{ingredient}:}
...
The result is $2D6-5$ Fire \glspl{ingredient}.
...
\begin{itemize}
  \item
  As \gls{fenestra} warms, female \glspl{hulk} give off a rancid stench, which can be smelled even outside cave mouths.
  \item
  \Glspl{hulk} are messy eaters, and often eat while moving.
  This leaves a trail of mosses, oozes, or \glspl{olm} picking through a path of morsels.
\end{itemize}
...
\begin{itemize}
  \item
  Ahead, \pgls{hulk} lies in wait for the troupe.
  ...
  \item
  \Pgls{hulk} sees the troupe and gives chase.
  ...
  \item
  \Pgls{hulk} is masticating a dead gnome's leg.
...
\index[mana]{Water!Gas-sack of watcher|glsentrytext{A}}
\paragraph{\Glspl{ingredient}:}
...
\begin{itemize}
  \item
  Dead humanoids, without a mark on their body.
  \item
  Motes of shining dust mean a watcher has scattered its seeds over the surrounding caverns.
\end{itemize}
...
\begin{itemize}
  ...
  Once the troupe sleep, the member taking watch begins hallucinating all the late-night encounters they have ever had coming to feed.
  ...
  The exit lies 40~\glspl{step} away.
  \item
  `\textit{Clang, clang, crunch!}' echoes out as \pgls{hulk} attacks nearby rocks.
  If the \glspl{pc} approach, it charges, and attacks, then turns around to strike a rock.
  If the troupe return later, they see the \gls{hulk} sitting still, as slow tentacles empty its shell.
  \item
  Someone taps a toe into the stagnant, metallic, lake, sending tiny ripples out.
  A moment later, larger ripples return, as if sending a response.

  A watchman lies under the water, `breathing' through its tentacles.
  The ripples come from its gas, rising as a great bubble.
\end{itemize}
