...
\begin{itemize}
  ...
  Have the party roll \roll{Wits}{Wyldcrafting} (\tn[8]) to notice the tremors, and their meaning in time.
...
Any time a basilisk finds chitincrawler eggs low enough to the ground, they eat them, and the chitincrawlers can do very little about it.
...
\index{Ingredients!Fate}
\paragraph{\Glspl{ingredient}:}
chitincrawler spinnerets (the web-spinning organs), at the back of the thorax, allows for \pglspl{boon} to the Fate Sphere.
The sticky substance must be spun on a loom (with \roll{Dexterity}{Crafts}), until it collapses into dust, at which point it provides $1D6$ uses.
...
\begin{itemize}
  ...
  \item
  The party reach a muddy slope in the road which they must descend carefully if they don't want to fall over (\roll{Dexterity}{Wyldcrafting}, \tn[7]).
  ...
  A dozen sleep under the snow, though the party will require an \roll{Intelligence}{Vigilance} roll (\tn[14]) to properly guess the number.
\end{itemize}
...
\begin{itemize}
  ...
  \ifodd\value{r4}
    ...
    The parent has left, so a \roll{Speed}{Athletics} roll allows a party member to get the eggs before a parent returns (\tn[9]).
  \else
...
\begin{itemize}
  ...
  \item
  One night on watch, a pack of wolves stalks the party. If the watchman fails the \roll{Strength}{Vigilance} check to stay alert, one wolf grabs the smallest party member's backpack and flees.
  \item
...
\begin{itemize}
  ...
  A \roll{Speed}{Athletics} check is required to follow the creature upwards, and only two party members will be able to approach at a time.
  \item
  ...
  \item
  The troupe make a \roll{Wits}{Vigilance} roll (\tn[10]) to notice a woodspy following them along the road from a great distance.
...
\begin{itemize}
  ...
  Have the party roll \roll{Wits}{Vigilance}, \tn[9] due to the twilight.
  \item
