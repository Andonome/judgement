...
But when over \gls{cOne}, before \gls{cTwo} brings snow, bears grow hungry, then reckless, as they prepare for hibernation.
...
\footnote{If this happens, replace any \gls{crawler} encounters with griffin encounters until after the next \gls{cThree}, double the number encountered, and give each a +2 bonus to Morale checks due to hunger.}
...
\paragraph{\Glspl{ingredient}:}
\gls{crawler} spinnerets (the web-spinning organs), at the back of the thorax, provide Fate Sphere \glspl{ingredient}.
