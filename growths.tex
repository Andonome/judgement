...
Each one corresponds to an Elemental \gls{sphere}, so \pgls{elixir} must be crafted from \pgls{ingredient} opposing \gls{sphere}.

The character requires a workshop (but might use basic cooking utensils around a campfire with a -1~Penalty) then rolls \roll{Intelligence}{Medicine} roll (\gls{tn} listed with each disease) along with one \gls{ingredient} of the correct Elemental \gls{sphere}.
