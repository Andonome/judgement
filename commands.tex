...
\newcommand\labelledDiceTrio[6]{
  ...
  \node (Left) at (2,2.5){(\arabic{age}) \Large #4};

  \node (Right) at (4,0){(\arabic{diceNo2}) \Large #5};

  \node (Top) at (0,0){(\arabic{diceNo}) \Large #6};
...
\newcommand\diceTrio[6]{
  ...
  \node (Left) at (2,2.5){\Large #4};

  \node (Right) at (4,0){\Large #5};

  \node (Top) at (0,0){\Large #6};
...
\newcommand\brochEvents{
  ...
  \item
  Dreameater (\vpageref{dreameater_moss}) moss covers the \gls{broch}'s sides, and someone's already had a nightmare.
...
\newcommand\encVillageFeatures{
  \begin{dlistDouble}
    ...
    The miller lives inside, and the ground floor is made from stone.
    \item
