...
\newcommand\encCivilization{%
  \ifnum\value{enc}<4%
    \Glsfmttext{broch} traders%
  \else%
    \ifnum\value{enc}<7%
      Outer \Glsfmttext{village} traders%
    \else%
      \ifnum\value{enc}<10%
        Inner \Glsfmttext{village} traders%
      \fi%
    \fi%
  \fi%
}
...
\newcommand\orrery{
  ...
  \begin{tikzpicture}
    ...
    \foreach \m/\d in {1/1,1/16}{
      ...
      \coordinate [label=\outline{\showCycle}] (FN) at (F)+(2.3,0.3);
    }
    %
