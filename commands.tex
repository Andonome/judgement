...
\newcommand\encPlant[1][\value{encnum}]{%
  \ifcase#1\relax%
  \or%
    \Glspl{bedshroom}
  \or%
...
\newcommand\encTownEvents{
  ...
  \Large\dicef{2} & \Large\dicef{4} &
              \Glspl{baker} are rebelling against the \gls{wheatGuild}, and have staged a walkout.
              Within a day, \pgls{curator} will slip poison into the drinks of the organizers. &
  ...
  \\
            ...
            & \Large\dicef{1} &
                        ...
                        \Pgls{tender} is raving at people to fetch him \pgls{mixer} for \pgls{elixir} for his illness (Mindflash Syndrome, \autopageref{Mindflash Syndrome}), but none will come since he insulted \pgls{helper} last year.
                        \\
  \hline
  \end{wideTable}
}
...
\newcommand\bothyEvents{
  ...
  The horses won't fit inside the \gls{bothy}, so let's start now, before the Sun sets!
  \fi
  \item
...
\newcommand\encVillageEvent{
  \begin{dlistDouble}
    ...
    \else
      ...
      \item
      The farmers hold a funeral for a fallen soldier. Everyone is drunk.
    \fi
    \item
    ...
    \item
    \Pgls{guard} has saved $1D6 \times 30$~\gls{sp}, and now hides in the \gls{village}, having abandoned his duties.
    \item
...
\newcommand\ngMissions{
  ...
  \begin{enumerate}
    ...
    \Pgls{ranger} says to
      \begin{dlist}
        ...
        \item
        check the perimeter -- just run round everywhere that's not forest.
      \end{dlist}
    ...
    \begin{dlist}
      ...
      \else
        ...
        They have \pgls{weight} of 20 each, so you can take this old donkey and cart to help
        (it carries \pgls{weight} of 60).
      \fi
      \item
    ...
    \begin{dlist}
      ...
      (something attacked a caravan, and the child fled).
      \item
      ...
      \item
      wounded \gls{ranger} to the nearest \gls{templeOfSickness}.
      \item
    ...
    \end{dlist}
    Return with confirmation, and you'll each get 3~\glspl{sp}.
    \item
    The \gls{jotter} says to
    \label{riderMissions}
    \begin{dlist}
      \item
      lay $D6$ traps around a nearby \gls{village}.
      Each has \pgls{weight} of 30, so take this donkey and cart and just try to work something out.
      \item
    ...
    \end{dlist}
    Return with confirmation, and you'll each get 5~\glspl{sp}.
    \item
    A travelling trader wants you to
      \begin{dlist}
        \item
        come guard his caravan on a journey to the next town.
        He'll pay a total of 20~\glspl{sp}.
        \item
        take this crossbow -- it should keep you safe.
        \item
        \ifodd\value{r4b}%
          talk about what the \gls{guard} are up to.
          He will join you on the road and pay 3~\glspl{sp} to one \gls{guard} to hear the `news' (but it had better be good).
        \else%
          join her going to the next town, and vouch that the bag of 200~\glspl{sp} belongs to the \gls{guard} and not her, so she doesn't have to pay a tax on it.
        \fi%
        \item
        ...
        The tradesman has just what the \gls{fiend} wants.
        She can pay a total of 60~\glspl{sp} if you return with a receipt.

        \textit{Nevermind `why', \gls{yonder} take ye!}
        \item
        break a friend out of jail in a nearby town.
        He had no idea those items were stolen!
        I reckon he can pay at least 20~\glspl{gp} once he's out.
      \end{dlist}
  ...
  \Pgls{thane} requests you seek out the
    ...
    \end{dlist}
    Complete the task and the \gls{thane} will owe you a favour.
  \item
  \Pgls{builder} orders you to capture a
    \begin{dlist}
      ...
      \item
      \gls{monster} for games in \pgls{court}; the \glspl{warden} want to see some good sport.
      \item
      ray of full moonlight.
      Some \glspl{doula} need it for something, but won't explain what they mean.
      \item
      thief in the act. He robs \glspl{warden} at night.
      Nobody has seen his face.
    \end{dlist}
    Don't fail, or the \gls{builder} will see you in the \gls{court}.
  \item
  The \gls{overseer} needs you to travel beyond the \gls{edge} and
    \begin{dlist}
    \item
    find where those goblins come from.
    \index{Goblins!Mission}
    \item
    ...
    \end{dlist}
    The \gls{templeOfBeasts} can pay 20~\glspl{sp} to the troupe's leader, and 30~\glspl{sp} to divide among the rest.
  \item
...
\newcommand\disease[5][]{
  \stepcounter{encnum}%
  \subsubsection[#2]{\twoDice{\theencnum}~#2}
  \index{#2|glsentrysymbol{eldren}}
  \label{#2}
  #3
  \par
  \textbf{Cure}: #4 (\tn[#5])
...
\newcommand\allEncounterTables{
  ...
  \begin{boxtable}[L|r|Lc]
    \Repeat{6}{%
      \addtocounter{enc}{-1}%
      \addtocounter{diceNo}{-1}
      \dicef{\value{diceNo}}~\encPlant[\value{diceNo}] &
