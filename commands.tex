...
\newcommand\brochEvents{
  ...
  \item
  A dozen \glspl{guard} archers sleep here already, with \pgls{jotter} upstairs.
  The tower doesn't have enough food, so the lowest ranking group has to leave.
  \item
  ...
  \item
  Dreameater moss (\vpageref{dreameater_moss}) covers the \gls{broch}'s sides, and someone's already had a nightmare.
  ...
  Deliveries should arrive soon, but in the meantime, someone needs to keep an eye on the holes in the roof, and make sure nothing crawls inside.
  \item
  The place has no piper, so you'll just have to sing till the beasts arrive!
  \end{dlist}
}
...
\newcommand\encVillageEvent{
  \begin{dlistDouble}
    ...
    \else
      \item
      A scout has destroyed a chitincrawler's nest and found an interesting item -- one \lootTalisman\ (\vpageref{talismanIndex}).
      \item
    ...
    \item
    A troupe of \gls{village} elders, armed with spears, return to report they have seen a fiend in the forest (\autoref{fiends}).
    \item
...
\newcommand\encVillageFeatures{
  \begin{dlistDouble}
    ...
    \item
      Hazel-nuts grow around the \gls{village}'s perimeter, as they keep other trees at bay.
...
\newcommand\ngMissions{
  ...
  \begin{enumerate}
    ...
    A travelling tradesman wants you to
      \begin{dlist}
        ...
        \item
        deliver something to a local fiend.
  ...
  Seek out the
    \begin{dlist}
      ...
      \item
      fiend with the lowest number and find out what you can (but don't get too close).
      \item
      ...
      \item
      local fiend, and find everything they can about it.
    \end{dlist}
  \item
  Capture a
    \begin{dlist}
    ...
    \end{dlist}
  \item
  \Pgls{thane} needs you to travel beyond the \gls{edge} and
    \begin{dlist}
    \item
...
\newcommand\showEnc[1][A]{%
  ...
  \needspace{3em}%
  \begin{center}%
  \textcolor{\pageSideColor}{\gls{beasts}}\qquad\textbf{Encounters}\qquad\textcolor{\pageSideColor}{\glsentrytext{#1}}
  \end{center}
}

\newcommand\funkyPlant[4][]{
  \subsubsection{#2}
  \index{#2~\glsentrytext{plant}}
  \label{#3}
  #4
  \ifstrempty{#1}{}{%
  \index[mana]{#1!#2~\glsentrytext{plant}}
  \par
