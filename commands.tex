...
\newcommand\diceTrio[6]{
  ...
  \node (A) at (0,0){(\arabic{age}) \Large #6};

  \node (B) at (4,0){(\arabic{enc}) \Large #5};

  \node (C) at (2,2.5){(\arabic{gold}) \Large #4};

  \draw[\pageSideColor] (A) to[bend left] node[left]{\large \dicef{#1}}(C);

  \draw[\pageSideColor] (C) to[bend left] node[right]{\large \dicef{#2}}(B);

  \draw[\pageSideColor] (B) to[bend left] node[below]{\large \dicef{#3}} (A);
