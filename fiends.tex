...
\noindent
Corpses which stave off decay continue to rot any time the soul exercises the body, so once \pgls{lich} creates itself, it begins with a fresh countdown to its own complete and natural death.
\Pgls{lich}'s activities revolve around staying cold and motionless.
...
Despite the stale mana in caves, \glspl{lich} can gather enough from the ambient death of small creatures to cast a \textit{Witness} \gls{spell} each \gls{cycle}, and discover a lot about their surroundings.
Many start to amass their power by acting as information brokers.
\Glspl{seeker} who pull at a source of information too much can find themselves face-to-face with a corpse which magically speaks directly into their minds.
...
\begin{dlist}
  ...
  \end{enumerate}
    \talisman{Blue-Blood Necklace}% Name
      ...
      Over \showOnset, the wearer becomes massive, and full of hunger, demanding \arabic{spellPlusOne} extra meals a day}% Summary
      {}% Details
    \showTalisman
  \item
...
\begin{dlist}
  ...
  After two \glspl{interval}, increase the dragon's Air~\gls{skill} by one.
  \item
...
\subsection{Plans}
\Glspl{dryad} fluctuate with the weather.
...
\Glspl{ogre} take charge because they're large.
...
\Glspl{ogre} are not born, but created by a `stuffing'.
...
\subsubsection{Goblin Metabolisms}
\label{goblinMetabolisms}
\index{Goblins!Metabolism}
demand a lot of upkeep.
They eat \pgls{ration} every \gls{interval} to avoid hunger.
They also need constant naps, as they sleep for two hours each \gls{interval}.
Losing sleep or failing to eat for two \glspl{interval} inflicts 3~\glspl{ep}.
But on the flip side, when goblins eat \pgls{ration} and sleep, they heal 3~\glspl{ep} at the \gls{interval}'s end;
and after two days of healthy living they heal a number of \glspl{hp} equal to half their current total.

Hobgoblins work the same way, but cannot miss a meal for \pgls{interval} so they must eat four meals a day.
\Glspl{ogre} eat five.

\newBeast{Hobgoblins}% Name
  {hobgoblin}% label
\index{Goblins!Hobgoblins}
\index{Hobgoblins|see {Goblins}}
...
\begin{dlist}
  ...
  \begin{enumerate}
    ...
    As a result, the \gls{griffin} population explodes during the next warm season (as noted \vpageref{crawlerBalance}).
  \end{enumerate}
  \item
  To lay siege to \pgls{broch}!
  Each \gls{interval} brings $2D6$ more goblins.
  \begin{enumerate}
    \item
