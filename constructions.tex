...
However, they can see a hidden light --- normally blocked by the walls of the flesh --- the light which comes from the mind itself;
a light which every soul emanates at all times.
...
When a living person approaches, these unfiltered minds see them like a beacon so bright that looking at it hurts.
...
\subsection{Communication \& Xenology}
\label{dead_communication}
\label{Xenology}
\index{Undead@\Glsentrytext{undead}!Communication}
\index{Artefacts@\Glsentryplural{artefact}!Communication}
...
\paragraph{The Xenology \gls{skill}}
involves the study of strange lands, alien thinking, paradoxical thoughts, and communication with everything where `empathy' would be a mistake.%
\footnote{Sometimes this `mistake' means danger, sometimes a category-error, and usually both.}

\begin{description}
  \item[\roll{Strength}{Xenology}]
    allows the speaker to become `brighter' (at least in the eyes of those who can see souls)
    in order to communicate clearer and farther (or just to attract a lot of excited \gls{undead}).
  \item[\roll{Intelligence}{Xenology}]
    lets someone understand what a constructed mind wants to say.
  \item[\roll{Wits}{Xenology}]
    gives insight into what a constructed mind intends or what kinds of things they can understand.
  \item[\roll{Charisma}{Xenology}]
    covers communicating one's intentions.
\end{description}

\Glsentrylongpl{tn} vary greatly.
\Pgls{ghast} trying to communicate hatred might be \gls{tn} 5 to understand and \tn[10] to explain a detailed plan about how to feed on more souls in the long-run by sparing the speaker now.
\Glspl{artefact}, in general, should have much higher \glspl{tn}, as those inhuman minds deal in fundamentally alien concepts, and each have their own idiosyncrasies.
You might begin by raising the \gls{tn} by 4 and then lowering it over time if the \gls{artefact} wants to make itself understood.
...
\index{Preservation!\Glsfmttext{undead}}
\index{Undead@\Glsentrytext{undead}!Preservation}
...
Each begins with a minimum Brawl \gls{skill} at +2.

\subsection{Specimens}

\newBeast{\Glsfmtplural{ghoul}}% Name
  {ghoul}% label
\index{Undead!Human}
...
The given example is a standard human \gls{ghoul}.
To make \glspl{ghoul} of other races, simply change the default Strength bonus -- dwarves have Strength 0, gnomes have Strength -2.

\ghoul[\npc{\D}{\Glsfmttext{ghoul}}]

\paragraph{Tactics:}
\Glspl{ghoul} attack in swarms and almost always grapple their targets as a first attack in order to claw and bite in later attacks.
...
\Glspl{ghoul} work in the same way.
...
\newBeast{\Glsfmtplural{ghast}}% Name
  {ghast}% label
\index{Undead!Human \Glsfmttext{ghast}}
...
\begin{itemize}
  ...
  \item
  \Pgls{ghast} kills travellers, then lays down with the dead, pretending to be one of their number.
  When the troupe arrive and examine the dead, it jumps the weakest member while they're off-guard.
  \item
  Using its ability to see the living at all times, \pgls{ghast} stalks the troupe from a long distance.
...
\Pgls{talisman} might wait for the right word to open a gateway; or look out for a man holding \pgls{torch}, so they can expend their energy on putting it out.
...
\begin{itemize}
  \item
  \Pgls{talisman} created to help someone hide with a Light spell will become \pgls{artefact} with the Fire and Earth \glspl{sphere}.
