...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item
    The second mission looks more promising: the troupe will need to investigate an area with \pgls{ogre}, some days' march away.
...
\mapentry{Mountains \& Coasts}
go onto the map first.
Mountains usually form in a line.

\begin{itemize}
  \item
  If only one point has a mountain, extend it with a long line of mountains, without touching any other point.
  \item
  If you have more points with mountains, connect them with a complete mountain range, going through them.
  \item
  If the mountain range cuts through other points, then those points sit in a valley.
  Give them some hills.
\end{itemize}
...
\begin{enumerate}
  \item
  At each mountain, draw a river to every neighbour without a mountain.
...
\begin{enumerate}
  \item
  The highest point without water becomes a bog.
  \item
  All other points without water receive some small hills.
  \item
...
Humans always litter the land with roads, and roads out always begin with \glspl{broch}.
...
\begin{enumerate}
  ...
  For every road over 10 miles long, place \pgls{bothy} $1D6+6$ miles from the lowest point.
...
For example, a map with a bog and three elvish settlements ranges might receive the name `Faebog', or one with two dragons and a lot of lakes might receive `Lochscale'.
...
\begin{enumerate}
  ...
  \item
  A wide lake spans the area, providing fish.
