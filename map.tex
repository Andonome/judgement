...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item
    The second mission looks more promising: the troupe will need to investigate an area with an ogre, some days' march away.
  ...
  \end{itemize}
  \item
  You'll need more information about that ogre, so you return here to \autoref{makingTheMap}, \autoref{mapCharacter}, to fill out more of the map.

  The dice say that the ogre has a set of complete plate armour, and a mushroom farm which extends down into the \gls{deep}.
  ...
  \item
  A single roll of $3D6$ tells you that the first road-encounter will come after 2~\glspl{interval}: a woodspy and rain.
  \item
  ...
  \item
  As the troupe bed down in a farmer's barn, the encounter occurs; the rain starts to fall while a woodspy has entered the \gls{village}: it begins feeling inside the barn for signs of an opening.

  Due to heavy rain, the troupe fail to wake, and the woodspy snatches up the smallest member, then carries them into the night\ldots
  \item
...
You can add human \glspl{village} and their inner roads later, in \autoref{mapCharacter}.
For now, the map only needs the long roads which connect two points, from \pgls{broch} to \pgls{broch}.

\begin{enumerate}
  \item
  At each human settlement, draw $1D6$ \glspl{broch}.
  They sit around 2 to 4 miles apart, guarding the \glspl{village} between them.
  \item
  Connect each human settlement to every other settlement with \pgls{lonelyRoad} connecting two distant \glspl{broch}.
  \begin{itemize}
    \item
  ...
  \end{itemize}
  \item
  If any road stretches for 50 miles or more, place \gls{lonelyTavern} half-way along that road (details \vpageref{lonelyTaverns}).
  \item
...
The \gls{campaign} begins on the smallest human settlement.%
...
\begin{itemize}
  ...
  \item
  If a neighbour has a lake, another warren arises with a grotto to lay traps for fish and eels.
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item
    Most \glspl{village} stay between \glspl{broch}, and all of them must be within 5 miles.
    \item
...
\setcounter{enumi}{2}
  ...
  \item
  The \gls{warden} is \pgls{witch}, and does not feel shy about using their skills.
\end{enumerate}
