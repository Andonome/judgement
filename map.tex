...
When any pair is more than 8, add 100 dwarves to the population.
...
\begin{exampletext}
  ...
  \labelledDiceTrio{2}{4}{3}{Polite Passage}{Tin Seam}{Magma Stream}
\end{exampletext}
...
\begin{enumerate}
  ...
  \item
  In the dark \gls{deep} below the mountain, extensive underground tunnels, filled with fungi, moss, living oozes, \glspl{olm}, and \glspl{hulk}.
...
\begin{exampletext}
  For example, a roll of `\dicef{4} \dicef{4} \dicef{6}' would show a powerful enchanter lives here, who tames \glspl{griffin} for the other elves to ride
  ...
  \labelledDiceTrio{4}{4}{6}{Mage Oak}{Peat-bog}{Peat-bog}
...
\begin{enumerate}
  ...
  Roll on the \nameref{plants} list \vpageref{plants}.
  ...
  The elder occasionally enchants \glspl{griffin} in the area, allowing a few elves to fly on their backs.
...
\label{gnollPoint}
have enough space for a small group to move around, following herds, then leaving parts to `rest' for a while.
...
Roll 3 dice and check every combination of 2 to find the characteristics of the gnolls who wander this ground.
...
\begin{enumerate}
  ...
  \index[Dye]
  \item
  One of the gnolls is a powerful \gls{witch}.
  \item
...
\begin{itemize}
  ...
  \item
  If \pgls{dryad} lives nearby, the gnomes create irritating songs which get stuck in your head, and sing them constantly.
  Dryads hate this kind of thing, and worry about getting an irritating song stuck in their head for the next few centuries, so they stay away from the gnomes.
  \item
...
\begin{dlist}
  ...
  (population: 100)
  \item
  Indirect democracy (people vote once a week, documentation is crucial, but often weaponized)
...
\subsection[Fiends]{\glsentrysymbol{fiend} Fiends}
\label{mapFiends}

\subsubsection{Bandit \Glsfmtplural{village}}
\label{banditsPoint}
\index{Bandits}
usually begin as stranded settlements.
\Pgls{village} left alone, with broken roads and the threat of starvation, has only one option when it comes to surviving without any supplies from a local town.
\Gls{fenestra}'s thick forests create a natural veil around any \gls{village} which wants to remain hidden.
...
A total of 200 humans live in the commune, but only 50 regularly exit to perform robberies.
...
They may live in deep caverns, shallow caverns, or sometimes just lay about in a Sunny patch of forest, without any roof.
In any case, they like to live close to mountains; surrounding trees can make flight difficult, while having a tall crag to leap from helps getting started.
\index{Flight!Dragons}
...
\label{dryadPoint}
\index{Dryads}
host strange and mutated plants and animals.
And dryads think `animals' includes most people.
...
\begin{dlist}
  ...
  \item
  Deep caverns exit in the centre of the \gls{dryad}'s lair.
  The \gls{dryad} sometimes journeys down, to bring up \glspl{hulk} and oozes, and observe them, then release them into the wild.

  These caverns go down into the \gls{deep}, and eventually connect to each other point which has caves.
  \item
  $1D3$ \glspl{basilisk} live with, and love, the \gls{dryad}.
  They will protect the \gls{dryad} with their lives.
  (find \glspl{basilisk} on \autopageref{basilisk})
  \item
  $1D6$ \glspl{griffin}, used as protectors and steeds.
  (find \glspl{griffin} on \autopageref{griffin})
\end{dlist}

\subsubsection{Hag Cottages}
\label{hagPoint}
\index{Hags}
look like any other cottage, except for the lack of walls, or other features of normal civilization.
In \gls{fenestra}, only one type of old lady lives alone.
...
\begin{dlist}
  \item
  The \gls{hag} has a dozen \glspl{griffin} circling her hut at all times.
...
\index{Ogres}
\index{Goblins!Lairs}
have $2D6 \times 5$ goblins, ready to raid any local settlements.
...
\label{lichPoint}
\index{Liches}
always include caverns.
Finding \pgls{lich} in the cold, twisting \gls{deep} makes a near-impossible challenge, as they change their exact location at random.
...
\begin{dlist}
  \item
  The \gls{lich} waited for centuries, building an army of \glspl{ghoul}.
  When a nearby human town weakened, the \gls{lich} attacked.

  It remains in the town to this day.
  The local \glspl{village} have rotted, but 3~\glspl{broch} remain standing.
  Create a lost city for the \gls{lich}, \vpageref{lostCities}.
  \item
  An old \gls{broch}, lost to the \gls{guard}, now used as an alchemical workshop.
  \item
  The \gls{lich} keeps a crew of $3D6$ \glspl{ghast}, ready to follow orders.
  \item
  A cabal of ten necromancers-in-training visit with bodies to feed the \gls{lich}'s army, in exchange for tutelage.
  They frequent local towns, and steal corpses whenever they can, to cart back to their master.
  \item
  When \pgls{basilisk} attacked, the \gls{lich} killed it, turning it into an undead steed.
...
\setcounter{enumi}{6}
  ...
  Sporadic arches mean that even \glspl{griffin} can struggle to leave quickly.
...
\subsection[Lost Cities]{\glsentrysymbol{yonder} Lost Cities}
\label{lostCities}
\index{Lost Cities}
...
If \pgls{lich} lives in the city, it has a lot of \glspl{ghoul}; if \pgls{ogre} king lives here, it has goblins.
...
\begin{dlist}
  ...
  A camp sits nearby, with two more \glspl{sunGuard}, six donkeys, and 20 days' \glspl{ration}.
...
\begin{itemize}
  ...
  Or if the city has \glspl{crawler} and \glspl{griffin}, they may see \pgls{griffin} caught in a web.
\end{itemize}
...
\begin{dlist}
  ...
  \item
  This tower once held the \gls{sunGuard}, but now has a single servant of the local fiend (usually goblins or \glspl{ghoul}).
...
\begin{enumerate}
  ...
  \textit{For example, a city with oozes and \glspl{griffin} would mean that if the \glspl{pc} make a noise, then an acidic ooze wanders out of a house, and heads towards them.
  Then on the next failed check to creep around, \pgls{griffin} would attack.}
  \item
  $2D6 + 8$ of the fiend's servants encounter them, and attack.

  \textit{Usually this means a lot of goblins, or a lot of \glspl{ghoul}.}
  \item
...
might seem like a great way to scout the entire city, but doing so leaves one visible to every dweller within.

Anyone climbing the outer walls gains a +2 Bonus to foraging rolls, but one of the dwellers with the highest number moves (or flies) to attack them.
...
can wait until you complete a few sessions.
As you go, check back at things you might add:
...
\begin{itemize}
  ...
  If not, you have just begun a `West Marches' style of \gls{campaign}, where the \glspl{pc} explore a grand, new wilderness.
  \index{West Marches}
  \begin{enumerate}
    \item
    Roll $1D6$ and draw \pgls{broch} between that point, and the map's edge.
    \item
    Set the Civilization Rating to 1 (details \vpageref{civilizationRating}).
    Only traders will come to grant food, and they will charge ten times the normal prices.
    If the \glspl{pc} ever fail to pay for food, then the Civilization Rating drops to 0 for three weeks.
  \end{enumerate}
  \item
...
\subsubsection{Connecting \Glsfmtplural{pc}}
\label{pcMapConnections}
to the map will help the players understand their context quickly, and provide a natural way to spend \glspl{storypoint}.
Make a list of $1D6$ possible past events, with a default and non-default race for each entry.
