...
\newcommand\guild[7]{
  \renewcommand\npcsymbol{\glsentrysymbol{#1}}
  \needspace{16em}
  \subsection{The Temple of #2}
  \gdef\subsectionList{}
  \gappto{\sectionList}{#2~\flourish~}%
  \index{Gods}
  \label{god:#2}
...
\guild{abderian}% Name
  {Poison}% Death
  {
    ...
    \index{Ale}%
    \index{Feasts}%
    \index{Poison!\expandafter\Glsentrytext{temple}}%
  }% God
  {poisons, rot, mould, fungus, ale, feasts}% Domain
  {alegar, distillation, rendering, and wine}% Defence
...
\subsubsection{The Structure}
emerges from letting people do what they do best.
None of the positions come with much of a rise in payment.

\begin{description}
  \item[\Glspl{mixer}]
  \glsentrydesc{mixer}
  \item[\Glspl{counter}]
  \glsentrydesc{counter}
  \item[The rest]
  help each other as they can.
  Once someone becomes sick, this \gls{temple} considers them part of the guild.
\end{description}
...
\guild{nulla}% Name
  {Misgenesis}% Death
  {
  \ifnum\value{temperature}=0\index{Nulla}%
  \else%
    \index{Nulla@\glsentryname{nulla}}%
  \fi%
  ...
  Only the \gls{doula} remember, as they pass her name from one to another, by entering each others' dreams, then writing the name on sand, one glyph at a time.}% God
...
Few \glspl{doula} practice \gls{witchcraft}, but those who do won't display it, so those who don't can hint that they can.
...
\begin{description}
  ...
  \Glspl{overseer} in the \gls{guard} pay them handsomely (but rarely in direct coinage) to seek out areas in the forest fit to built \pgls{village}.
...
\guild{paik}% Name
  {Justice}% Death
  {
    Entering the pit, the executioner leans in to whisper, ``\textit{just don't make a fuss, give them a good show, okay?
    And try to smile, but not too much}''.

    The noose hangs high above the pit, waiting for stage-hands to lower it.
    An iron gate in the wall stands quietly, so the jurors can guess what waits inside.
    Some of the crowd sneer, others wave down through the bars; like a greeting, or perhaps just calling for you to meet their eyes before you go.
    A lot of the jurors like that.

    The \glsentrytext{warden} remains silent.

    \begin{speechtext}
      ``My \gls{warden}, the man accusing me of theft was in fact looting the bodies when I arrived.
      I believe I know where he buried the items, if only someone would journey with me as a witness.
      I watched him dig there, shortly after he\ldots''
    \end{speechtext}

    {\sffamily ``You like to\ldots\emph{watch}?''}

    The crowd smirks, then laughs as the jester climbs the cage-dome above the pit, in a mocking skulk, eyeing the crowd dramatically.
    ...
    The \gls{warden} tells the jester to be quiet and motions towards the iron gate.
    The \gls{sunGuard} draw it up, slowly.
    The crowd squeeze forward, quietly, and a woman steps out.
...
\Glspl{warden} form the only \gls{temple} which actively kills, though they remind people that society must have laws, and that they attempt to limit deaths by Justice to an absolute minimum.
...
\begin{description}
  \item[Jesters]
  are not formally ranked in any way, but they make the accusations in court, so everyone knows and speaks politely to a jester when they see one on the streets.
...
\index{Pit of Justice|see {Trials}}
\label{pitOfJustice}
...
\index{Theatre of Oaths|see {Trials}}
\index{Trials}
...
\guild{sable}% Name
  ...
  {clothing, fire, vinegar, salt, glass, and savings}% Defence
...
\begin{description}
  ...
  They often sleep outside, at the foot of remote towers.
  \index{Towns!Entry}

  \item[\Glspl{gDigger}]
  \glsentrydesc{gDigger}.
  And in practice, \glspl{guard} don't make a kill alone, so groups will take it in turns to present \pgls{monster} they `killed', just to make sure everyone gains a rank.

  \Glspl{gDigger} can only gain further rank once they repay their debt to the \gls{templeOfBeasts}.
  Until then, they widen roads, keep the \glspl{broch} tidy, and keep guard over any nearby \glspl{village} that report sighting \pgls{monster}.

  \item[\Glspl{soldier}]
  often begin as farmers who kill \pgls{monster}, then decide to work as \pgls{guard}.
  They have no restrictions on their movement, except that they have to follow orders from the \gls{temple}.
  Most of their time is spent walking with trader caravans.

  \item[\Glspl{cutter}]
  have journeyed past the \gls{edge} to complete a mission, perhaps ridding \pgls{village} of \glspl{griffin} by finding their nest, or tracking \pgls{basilisk} back to the cave where it hibernates.

  \item[\Glspl{ranger}]
  \label{ranger}
  \glsentrydesc{ranger}, which usually means killing bandits.
  Their existence around the \glspl{broch} provides a warning to all \glspl{fodder} not to flee their post.

  \item[\Glspl{jotter}]
  \label{jotter}%
  need only show their literacy, and some basic organizational skills.
  \Glspl{overseer} select \pgls{jotter} from the \glspl{cutter} and \glspl{ranger} as needed.

  \Glsentrylongpl{pc} should never achieve this rank.

  \item[\Glspl{builder}]
  organize the lower ranks to build new \glspl{broch}, often working with \glspl{wayfinder} and \glspl{cartographer}.

  Any \gls{cutter} or \gls{ranger} (or, in theory, \gls{jotter}) can achieve this rank by locating three patches of valuable plants which can used as \glspl{ingredient}, and returning the haul to \pgls{broch}.

  \item[\Glspl{thane}]
  \label{thanes}
  must return with three captured \glspl{monster} in order to gain this rank.
  They lead \glspl{ranger} on dangerous missions, and demand payment from the \gls{temple} for their services.

  Reaching this rank means a light retirement, as they no longer have any obligations, besides the 50\% cut due to the \gls{temple} for any \gls{monster}-bodies sold.
...
When building \pgls{broch}, one should start with a quarry, a stream, and a view of two \glspl{village}, but \glspl{overseer} will accept two out of three.
...
\guild{yonder}% Name
  {Curiosity}% Death
  {
    ...
    \index{Soap}%
    \index{Cartography}%
    \index{Torches@\Glsfmtplural{torch}}%
  }% God
  {light, candles, \glspl{torch}, paper, cartography, signage, soap, and secrets}% Domain
  {knowledge}% Defence
...
If this seems suspiciously indeterminate, then the reader has stumbled into one of \gls{fenestra}'s theological debates.
...
\begin{description}
  \item[\Glspl{chandler}]
  \glsentrydesc{chandler}.
  \guildRank{Scribblers}
  copy books.
  Anyone can begin as a scribbler, if they answer a riddle (and everyone may receive only one riddle per year, to be answered on the spot).
  \item[\Glspl{scrivener}]
  \glsentrydesc{scrivener}.
  \guildRank{Wanderers}
  ...
  \glsentrydesc{librarian}.
  \item[\Glspl{philosopher}]
  \glsentrydesc{philosopher}.
  \ifnum\value{cycle}=2
...
\footnote{`\nameref{warOfLies}' is covered \vpageref{warOfLies}.}
